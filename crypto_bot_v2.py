...
while True:  # Fetch historical data
    ...
    if (
        np.sum(
            [
                (df["macd"].iloc[-1] > df["macd_signal"].iloc[-1] + 7),
                (df["rsi"].iloc[-1] > 50 + 2),
                (df["close"].iloc[-1] > df["bollinger_middle"].iloc[-1] * 1.0007),
                (df["stoch_k"].iloc[-1] > df["stoch_d"].iloc[-1] + 7),
                (df["adx"].iloc[-1] > 25 + 7),
            ]
        )
        ...
        np.sum(
            [
                (df["macd"].iloc[-1] < df["macd_signal"].iloc[-1] - 7),
                (df["rsi"].iloc[-1] < 50 - 2),
                (df["close"].iloc[-1] < df["bollinger_middle"].iloc[-1] * 0.9993),
                (df["stoch_k"].iloc[-1] < df["stoch_d"].iloc[-1] - 7),
                (df["adx"].iloc[-1] < 25 - 7),
            ]
        )
