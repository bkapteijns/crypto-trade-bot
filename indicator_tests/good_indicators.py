...
for i in range(100, 1000):
    if np.sum([(df['macd'][i] > df['macd_signal'][i]), (df['rsi'][i] > 50), (df['close'][i] > df['bollinger_middle'][i]), (df['stoch_k'][i] > df['stoch_d'][i]), (df['adx'][i] > 25)]) > n_indicators - aggression:
        # Buy signal
        if(not long):
            long = True
            short = False
            while (bitcoins < 1):
                ...
                buy(i)
    elif np.sum([(df['macd'][i] < df['macd_signal'][i]), (df['rsi'][i] < 50), (df['close'][i] < df['bollinger_middle'][i]), (df['stoch_k'][i] < df['stoch_d'][i]), (df['adx'][i] < 20)]) > n_indicators - aggression:
        # Sell signal
        if (not short):
            long = False
            short = True
...
plt.plot(profits)
plt.title("Profits from the good indicators")
plt.show()
