...
for i in range(len(df)):  # Fetch historical data
    ohlcv = exchange.fetch_ohlcv(symbol, "1m")

    if (
        np.sum(
            [
                (df["macd"][i] > df["macd_signal"][i] + 7),
                (df["rsi"][i] > 50 + 2),
                (df["close"][i] > df["bollinger_middle"][i] * 1.0007),
                (df["stoch_k"][i] > df["stoch_d"][i] + 7),
                (df["adx"][i] > 25 + 7),
            ]
        )
        ...
        np.sum(
            [
                (df["macd"][i] < df["macd_signal"][i] - 7),
                (df["rsi"][i] < 50 - 2),
                (df["close"][i] < df["bollinger_middle"][i] * 0.9993),
                (df["stoch_k"][i] < df["stoch_d"][i] - 7),
                (df["adx"][i] < 25 - 7),
            ]
        )
        ...
        long = False
        short = False
    values[i] = capital + bitcoins * df["close"][i]
