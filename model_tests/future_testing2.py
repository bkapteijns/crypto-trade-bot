...
threshold = 0.1   # What percentage change is valid
...
profits = np.zeros((3, (int(len(df)/3)+1), 10))
for future in np.arange(5, 55, 5):
    ...
    for f, (train_indices, test_indices) in enumerate(kf.split(X, y)):
        ...
        for i in range(len(test_indices)):
            if (model.predict(X[[test_indices[i]]]) == 1 and bitcoins[int(future/5-1)] != 1):
                buy(int(future/5-1), X[test_indices[i]][0])
            elif (model.predict(X[[test_indices[i]]]) == -1 and bitcoins[int(future/5-1)] != -1):
                sell(int(future/5-1), X[test_indices[i]][0])
            elif (bitcoins[int(future/5-1)] == 1):
                sell(int(future/5-1), X[test_indices[i]][0])
            elif (bitcoins[int(future/5-1)] == -1):
                buy(int(future/5-1), X[test_indices[i]][0])
            profits[f, i, int(future/5-1)] = capital[int(future/5-1)] + \
                bitcoins[int(future/5-1)]*X[test_indices[i]][0]
        capital = np.array([30000]*10)
        bitcoins = np.zeros(10)
...
for future in np.arange(5, 55, 5):
    plt.plot(range(int(len(df)/3)+1),
             profits[:, int(future/5-1)], label=str(future))
plt.legend()
plt.show()
