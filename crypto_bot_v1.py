...
while(True):  # Fetch historical data
    ...
    if np.sum([(df['macd'].iloc[-1] > df['macd_signal'].iloc[-1]), (df['rsi'].iloc[-1] > 50), (df['close'].iloc[-1] > df['bollinger_middle'].iloc[-1]), (df['stoch_k'].iloc[-1] > df['stoch_d'].iloc[-1]), (df['ema_12'].iloc[-1] > df['ema_26'].iloc[-1])]) > n_indicators - aggression:
        # Buy signal
        if(not long):
            long = True
            short = False
    ...
    elif np.sum([(df['macd'].iloc[-1] < df['macd_signal'].iloc[-1]), (df['rsi'].iloc[-1] < 50), (df['close'].iloc[-1] < df['bollinger_middle'].iloc[-1]), (df['stoch_k'].iloc[-1] < df['stoch_d'].iloc[-1]), (df['ema_12'].iloc[-1] < df['ema_26'].iloc[-1])]) > n_indicators - aggression:
        # Sell signal
        if (not short):
            long = False
            short = True
